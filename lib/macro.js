// The rest of the market: VIX, the 10-year yield, oil, gold, and the
// dollar — the five numbers that explain most "why is everything red"
...
// Crypto: the four largest liquid coins, same keyless Yahoo endpoint.
// Crypto trades around the clock; Yahoo's daily bars roll at midnight UTC,
// so the change shown is "since the prior UTC daily close" — stated on the
// card rather than passed off as a rolling 24h figure.
const CRYPTO = [
  { sym: "BTC-USD", label: "Bitcoin", kind: "usd", hint: "BTC — spot price in US dollars" },
  { sym: "ETH-USD", label: "Ethereum", kind: "usd", hint: "ETH — spot price in US dollars" },
  { sym: "SOL-USD", label: "Solana", kind: "usd", hint: "SOL — spot price in US dollars" },
  { sym: "XRP-USD", label: "XRP", kind: "fx", hint: "XRP — spot price in US dollars" },
];

let cryptoCache = null;

export async function getCrypto() {
  if (cryptoCache && Date.now() - cryptoCache.at < TTL_MS) return cryptoCache.rows;
  const settled = await Promise.allSettled(CRYPTO.map(quoteOne));
  const rows = settled.map((r) => (r.status === "fulfilled" ? r.value : null)).filter(Boolean);
  if (rows.length) cryptoCache = { at: Date.now(), rows };
  return rows;
}
