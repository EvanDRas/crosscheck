...
export async function pointInTimeCall(ticker, D) {
  ...
  if (Date.parse(D) - Date.parse(dRow.date) > 14 * DAY) {
    throw new Error(`price data for ${ticker} stops at ${dRow.date} — likely delisted or renamed by ${D}`);
  }
  if (rows[rows.length - 1].date === dRow.date && Date.now() - Date.parse(dRow.date) > 14 * DAY) {
    throw new Error(`price data for ${ticker} stops at ${dRow.date} — nothing to grade the call against`);
  }
