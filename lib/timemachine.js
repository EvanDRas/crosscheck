...
// Intentional refusals (bad date, delisted ticker, no filings) carry this
// class so the route can tell them from genuine upstream failures.
...
export async function pointInTimeCall(ticker, D) {
  ...
  if (!rows || rows.length < 150) throw new PitError("not enough price history for that ticker/date");
  const dRow = rowOn(rows, D);
  if (!dRow) throw new PitError(`no price on/before ${D}`);
  // A big gap between the requested date and the last trade means the ticker
  // stopped trading (delisted/renamed) — refuse rather than render a
  // confident-looking 0.0-year, +0.0% outcome.
  if (Date.parse(D) - Date.parse(dRow.date) > 14 * DAY) {
    throw new PitError(`price data for ${ticker} stops at ${dRow.date} — likely delisted or renamed by ${D}`);
  }
  if (rows[rows.length - 1].date === dRow.date && Date.now() - Date.parse(dRow.date) > 14 * DAY) {
    throw new PitError(`price data for ${ticker} stops at ${dRow.date} — nothing to grade the call against`);
  }
  ...
  if (!factsAll) throw new PitError("not an SEC filer");
  ...
  if (sharesInst && dRow.close != null) {
    const fRow = rowOn(rows, sharesInst.end);
    if (fRow?.close != null && fRow.adj && dRow.adj) {
      let factor = (fRow.close / fRow.adj) / (dRow.close / dRow.adj);
      if (!Number.isFinite(factor) || factor <= 0) factor = 1;
      marketCapUsd = sharesInst.val * factor * dRow.close;
    } else {
      // Share-count filing predates the price window: a split between filing
      // and D would silently mis-scale market cap by the full split ratio —
      // degrade honestly instead of guessing factor 1.
      notes.push("share count predates available price history — valuation category degraded");
    }
  } else {
  ...
  if (!edgar) throw new PitError(`filings too stale/absent as of ${D} (SEC XBRL coverage starts ~2010)`);
