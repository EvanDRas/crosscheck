...
// Optional research datasets, configured purely via env (see .env.example) —
// no paths ship in the repo. Absent config just means no local chart source.
const PRICES_PARQUET = process.env.HISTORY_PRICES_PARQUET || "";
const TIINGO_DIR = process.env.HISTORY_TIINGO_DIR || "";
...
export function gradeFromPanel(pairs) {
  return new Promise((resolve, reject) => {
    ...
    // A listener-less stream 'error' (bad PYTHON_BIN, early exit) is an
    // uncaught exception that would kill the whole server — swallow it here;
    // the 'error'/'close' handlers above already reject with the real cause.
    child.stdin.on("error", () => {});
    try {
      child.stdin.write(JSON.stringify({ pairs }));
      child.stdin.end();
    } catch { /* stream already destroyed — close handler reports it */ }
  });
}
