...
export async function feedNews({ tab, apiKey, limit = 10, tickers = [], names = {} }) {
  ...
  if (tab === "briefing") {
    ...
    const keep = scored.filter((it) => {
      ...
      // impact >= 2 bar below, so thin single-outlet items don't flood in.
      const megaEarn = IMPACT_MEGA.test(it.headline) && IMPACT_EARNINGS.test(it.headline);
      if (companyMentions(it.headline) === 1 && !macro && !megaEarn) return false;
    ...
    if (out.length < 10) {
      ...
      for (const it of scored) {
        ...
        if (companyMentions(it.headline) === 1 && !IMPACT_MAJOR.test(it.headline) &&
            !(IMPACT_MEGA.test(it.headline) && IMPACT_EARNINGS.test(it.headline))) continue;
        out.push(it);
        seen.add(it.link);
      }
    }
