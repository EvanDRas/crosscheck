...
export async function marketNews(apiKey) {
  const sources = [
    ["Google News", (async () => {
      ...
      return items.map((it) => {
        ...
        return { headline, source, date: toIso(text(it.pubDate)), link: text(it.link), summary: "" };
      });
    })()],
  ];
  if (apiKey) {
  ...
  return dedupeAndSort(items.filter((it) => it.headline && !SPAM.test(it.headline)), 10, 4);
}
