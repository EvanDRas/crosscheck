...
export function stripHtml(html) {
  return String(html ?? "")
    .replace(/<[^>]*>/g, " ")
    .replace(/&#[xX]([0-9a-fA-F]+);/g, (_, n) => { const cp = parseInt(n, 16); return cp > 0 && cp <= 0x10FFFF ? String.fromCodePoint(cp) : " "; })
...
export function isRelevant(item, terms) {
  ...
  // Word-bound like the ticker path — "Snapple" must not hit "apple". No 'i'
  // flag: lower and the phrases are already lowercased. \b would break on
  // cores starting/ending with '&' or '-'.
  if (terms.phrases.some((p) => new RegExp(`(^|[^a-z0-9])${escapeRe(p)}([^a-z0-9]|$)`).test(lower))) return true;
...
export function dedupeAndSort(items, limit = MAX_ITEMS, perSourceCap = 5) {
  // Normalize once for every feed path: strip a dangling "- Source" suffix
  // (the source is already printed in the meta line) and blank summaries
  // that merely repeat the headline — no card should say the same sentence
  // twice.
  const norm = (x) => String(x ?? "").toLowerCase().replace(/[^a-z0-9]+/g, " ").trim();
  for (const it of items) {
    if (it.headline && it.source) {
      const stripped = it.headline.replace(new RegExp(`\\s*[-\u2013|]\\s*${escapeRe(it.source)}(\\.com)?\\s*$`, "i"), "").trim();
      if (stripped) it.headline = stripped;
    }
    if (it.summary && (norm(it.summary) === norm(it.headline) || norm(`${it.headline} ${it.source ?? ""}`).startsWith(norm(it.summary)))) {
      it.summary = "";
    }
  }
  ...
  for (const it of usable) {
    ...
    if (dupe) {
      ...
      // Count only distinct outlets within 48h — one source's recurring
      // template headline must not manufacture a fake "N outlets" badge.
      // The dupe LOOKUP stays unwindowed so the repeats still collapse.
      if (!dupe._srcs.has(it.source) &&
          Math.abs((Date.parse(dupe.date ?? 0) || 0) - (Date.parse(it.date ?? 0) || 0)) < 48 * 3600e3) {
        dupe._srcs.add(it.source);
        dupe.covered = (dupe.covered ?? 1) + 1;
      }
      continue;
    }
    // Keep scanning after the list fills: syndicated copies further down the
    // newest-first pool still merge into kept (covered stays honest), they
    // just can't create new entries.
    if (kept.length >= limit) continue;
    const entry = { ...it, _tokens: tokens, _srcs: new Set([it.source]) };
  ...
  return kept.map(({ _tokens, _srcs, ...it }) => it);
}
...
async function fromWires(kind) {
  ...
  // A full-failure result is cached for 60s (backdated) so an offline or
  // blocked network fails fast instead of re-hanging every request for 9s.
  wireCache.set(kind, { at: items.length ? Date.now() : Date.now() - (10 * 60_000 - 60_000), items });
  return items;
}
...
const IMPACT_MAJOR = /\b(fed|federal reserve|powell|rate (cut|hike|decision)|interest rates?|inflation|cpi|ppi|jobs report|payrolls|unemployment|gdp|recession|tariffs?|trade war|opec|crude|oil price|treasur(y|ies)|yields?|debt ceiling|(sovereign|debt|bond|loan) defaults?|defaults? on (its |their )?(debt|loans?|bonds?)|bank (failure|run)|(market|stock|flash) crash|selloff|sell-off|circuit breaker|war\b(?! (memorial|museum))|sanctions|government shutdown|stimulus)\b/i;
...
const TIER1_SOURCE = /^(reuters|associated press|bloomberg|the wall street journal|wsj|cnbc|financial times|barron|marketwatch|federal reserve|the new york times)/i;
...
// Pure: rank the briefing from a deduped, impact-scored pool. Exported for
// tests — the churn filter (single-company stories out), its mega-cap
// earnings exemption, and the thin-day backfill must not regress silently.
export function assembleBriefing(scored, names, limit) {
  // Count distinct COMPANIES, not alias strings — "Alphabet's Google" is one
  // firm, not two. Regexes compiled once per request, not per headline.
  const aliasRes = Object.values(names).map((as) => as.map((a) => new RegExp(`\\b${escapeRe(a)}\\b`, "i")));
  const companyMentions = (h) => aliasRes.filter((res) => res.some((re) => re.test(h))).length;
  // Mega-cap earnings are the exact stories the impact score exists to
  // rank — the churn filter must not delete them. They still face the
  // impact >= 2 bar, so thin single-outlet items don't flood in.
  const isChurn = (h) => companyMentions(h) === 1 && !IMPACT_MAJOR.test(h)
    && !(IMPACT_MEGA.test(h) && IMPACT_EARNINGS.test(h));
  const keep = scored.filter((it) => !isChurn(it.headline) && (it.impact >= 2 || IMPACT_MAJOR.test(it.headline)));
  ...
  if (out.length < 10) {
    ...
    for (const it of scored) {
      ...
      if (seen.has(it.link) || isChurn(it.headline)) continue;
      out.push(it);
      seen.add(it.link);
    }
  }
...
// Pure: assemble the front-page list from the two deduped lanes — up to two
// reserved photo slots (their covered/summary reconciled against the deeper
// main lane), per-feed caps, then backfill. Exported for tests.
export function assembleMarketList(merged, photos, limit) {
  ...
  for (const it of photos) {
    ...
    // The photo lane dedupes over far fewer slots, so its covered count lags
    // the main lane's — serve the merged counterpart's numbers (and summary),
    // not the lane's undercounted copy.
    const twin = merged.find((m) => m.link === it.link || isNearDuplicate(titleTokens(m.headline), titleTokens(it.headline)));
    if (twin) {
      it.covered = Math.max(it.covered ?? 1, twin.covered ?? 1);
      if (!it.summary && twin.summary) it.summary = twin.summary;
    }
    take(it);
  }
...
export async function feedNews({ tab, apiKey, limit = 10, tickers = [], names = {} }) {
  ...
  if (tab === "briefing") {
    ...
    return assembleBriefing(scored, names, limit).map(({ feed, ...it }) => it);
  }
  const wantMarket = tab === "markets";
  const [g, f, wr] = await Promise.allSettled([
    fromGoogleTopic(wantMarket ? "BUSINESS" : "WORLD"),
    // Finnhub's general feed is business news — a headline merely lacking a
    // market keyword is not world news. Markets tab only; the world tab
    // skips the fetch (and the API call) entirely.
    apiKey && wantMarket ? fromFinnhubGeneral(apiKey, 60) : Promise.resolve([]),
    wantMarket ? Promise.all([fromWires("markets"), fromWires("economy")]).then((a) => a.flat()) : Promise.resolve([]),
  ]);
  const items = [
    ...(g.status === "fulfilled" ? g.value : []),
    ...(f.status === "fulfilled" ? f.value.filter((it) => MARKET_RE.test(it.headline)) : []),
    ...(wr.status === "fulfilled" ? wr.value : []),
  ].filter((it) => it.headline && !SPAM_RE.test(it.headline) && !FLUFF_RE.test(it.headline));
  // Cap any single source at 6 visible slots regardless of page depth — one
  // over-eager wire must never own the whole tab.
  return dedupeAndSort(items, limit, 6).map(withImpact).map(({ feed, ...it }) => it);
}
...
export async function marketNews(apiKey, limit = 10) {
  ...
  return assembleMarketList(merged, photos, limit).map(({ feed, ...it }) => it);
}
