...
const TIER1_SOURCE = /^(reuters|associated press|bloomberg|the wall street journal|wsj|cnbc|financial times|barron)/i;
...
export async function feedNews({ tab, apiKey, limit = 10, tickers = [], names = {} }) {
  ...
  if (tab === "briefing") {
    ...
    const [g, f, w] = await Promise.allSettled([
    ...
    return keep.slice(0, Math.min(limit, 12)).map(({ feed, ...it }) => it);
  }
