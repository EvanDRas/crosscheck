...
export async function feedNews({ tab, apiKey, limit = 10, tickers = [] }) {
  if (tab === "top") return marketNews(apiKey, limit);
  if (tab === "you") {
    const settled = await Promise.allSettled(tickers.slice(0, 6).map(async ({ ticker, name }) => {
      const items = await fromGoogleNews(ticker, name ?? ticker);
      const terms = relevanceTerms(ticker, name ?? "");
      const canFilter = terms.phrases.length || terms.tokens.length || terms.ticker.length >= 2;
      return (canFilter ? items.filter((it) => isRelevant(it, terms)) : items).slice(0, 8).map((it) => ({ ...it, tickers: [ticker] }));
    }));
    const items = settled.flatMap((r) => (r.status === "fulfilled" ? r.value : []));
    return dedupeAndSort(items.filter((it) => it.headline && !SPAM_RE.test(it.headline)), limit, 4);
  }
  ...
  return dedupeAndSort(items, limit, Math.max(3, Math.ceil(limit / 4))).map(({ feed, ...it }) => it);
}
