...
// Keyless company search from the SEC's own ticker map: name-to-ticker
// lookups work before any vendor key exists. Ticker-prefix matches rank
// first, then company-name substrings.
export async function searchCompanies(q) {
  const needle = String(q ?? "").trim().toUpperCase();
  if (needle.length < 2) return [];
  const map = await getCikMap();
  const display = (t) => t.replace(/-([AB])$/, ".$1");
  const okShape = (t) => /^[A-Z]{1,6}(-[AB])?$/.test(t);
  const byTicker = [];
  const byName = [];
  for (const [ticker, row] of Object.entries(map)) {
    if (!okShape(ticker)) continue;
    if (ticker.startsWith(needle)) byTicker.push({ symbol: display(ticker), name: row.name });
    else if (String(row.name).toUpperCase().includes(needle)) byName.push({ symbol: display(ticker), name: row.name });
  }
  byTicker.sort((a, b) => a.symbol.length - b.symbol.length);
  return [...byTicker, ...byName].slice(0, 6);
}
...
export async function getCompanyFacts(ticker) {
  ...
  // Each entry is a full companyfacts document (5-20MB parsed) — 60 of them
  // could pin hundreds of MB of heap for the 12h TTL. A dozen covers a
  // browsing session; older ones refetch cheaply.
  if (factsCache.size > 12) factsCache.delete(factsCache.keys().next().value);
  return facts;
}
...
export function computeTrajectory(facts) {
  ...
  if (shares.length >= 2) {
    ...
    // Same staleness discipline as the metrics guard: a share series that
    // ended years ago must not render as a current buyback/dilution claim.
    if (years >= 0.9 && base.val > 0 && toT(latest.end) >= Date.now() - 450 * DAY) {
      dilution = {
