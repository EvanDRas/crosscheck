...
export async function analyzeTicker(ticker, { apiKey, log = true, includeNews = true } = {}) {
  ...
  let tiingoChartErr = null;
  if (!history?.updatesDaily && hasTiingoKey()) {
    try {
      const th = await getTiingoHistory(ticker);
      if (th) history = th;
    } catch (err) {
      // Free-tier limits must never break the page — but a silently missing
      // chart looks broken. Remember why, so the card can say so.
      tiingoChartErr = err?.message ?? "error";
    }
  }
  ...
  if (factsDoc) {
    ...
    if (edgar) {
      ...
      if (warnings.length > before) {
        // Teach, don't just warn: the usual cause is a fresh filing the
        // vendor hasn't processed yet, and the filing is the primary source.
        warnings.push(`Why sources can differ: the SEC number comes straight from the company's own filings${edgarThrough ? ` (data through ${edgarThrough})` : ""}, while vendors standardize line items and often lag a just-filed report by days. When they disagree right after a new filing, the filing is usually the fresher number.`);
      }
    }
  }
  ...
  const payload = {
    ...
    history,
    // Why the chart is missing, when it is and we know. Tiingo's hourly cap
    // is shared with track-record grading, so cold starts can spend it.
    historyNote: !history && tiingoChartErr
      ? (/429|limit|hourly|allocation/i.test(tiingoChartErr)
        ? "No chart this load — the free Tiingo tier's hourly limit is spent (grading the track record shares the same budget). It resets within the hour and the chart comes back on its own."
        : `No chart this load — Tiingo couldn't deliver prices (${tiingoChartErr}).`)
      : null,
    analystTrends,
    analystHistory,
    earnings,
  ...
  if (log && apiKey && scoring.verdict && hasQuote && !payload.degraded && scoring.availableCount >= 4) {
    try {
      ...
      payload.logged = await logVerdict({
        t: payload.asOf,
        date: marketDate(),
        ticker,
        price: quote.c,
