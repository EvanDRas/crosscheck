...
export async function getTiingoDaily(ticker, startDate) {
  ...
  // Cache key includes the window — a short grading fetch must never be
  // served as a ticker's "entire history" to the chart (or vice versa).
  const ck = `${t}|${startDate}`;
  const hit = cache.get(ck);
  if (hit && Date.now() - hit.at < TTL_MS) return hit.rows; // rows may be null — a cached "Tiingo doesn't know this one"
  ...
  const remember = (rows) => {
    // Negative results cache too: an unknown ticker used to re-spend an API
    // call (and a fetch slot) on every single load, forever.
    cache.set(ck, { at: Date.now(), rows });
    if (cache.size > 80) cache.delete(cache.keys().next().value);
    return rows;
  };
  if (res.status === 404) return remember(null); // unknown ticker
  ...
  const rows = (Array.isArray(doc) ? doc : [])
    ...
    .map((r) => ({ date: String(r.date).slice(0, 10), adj: r.adjClose, close: typeof r.close === "number" ? r.close : null }));
...
// Grading legs for many ledger dates of one ticker from a single series
// fetch. The per-pair version cost one fetch per (ticker, date) and could
// never catch up once the ledger grew past the per-load cap — the batch
// logs ~50 new pairs a day. startDate is anchored to the EARLIEST date so
// the cache key stays stable across loads (the ledger is append-only).
export async function tiingoGradeMany(ticker, anchorDates) {
  if (!anchorDates.length) return {};
  const earliest = anchorDates.reduce((a, b) => (a < b ? a : b));
  const from = new Date(Date.parse(earliest) - 14 * DAY).toISOString().slice(0, 10);
  const rows = await getTiingoDaily(ticker, from);
  if (!rows) return {};
  ...
  for (const d of anchorDates) {
    let anchor = null;
    for (const r of rows) {
      if (r.date <= d) anchor = r;
      else break;
    }
    if (anchor) out[d] = { anchorDate: anchor.date, anchorClose: anchor.adj, latestDate: latest.date, latestClose: latest.adj };
  }
  return out;
}
