...
// v2 bands are PERCENTILE-CALIBRATED, not hand-set: cutoffs are drawn from
// the empirical score distribution (16,497 point-in-time S&P calls 2011-2024,
// scripts/backtest_pit_full.py, cross-checked against the live ledger). The
// median large cap scores ~60-62 on these anchors, so v1's BUY>=58 labeled
// the AVERAGE company a buy. Under v2 a verdict states rank: STRONG BUY =
// top ~10% of large caps on these metrics, BUY = top ~30%, HOLD = middle,
// SELL/STRONG SELL = bottom ~30%/~10%. Rank, not prophecy — see EVIDENCE.md.
...
// near-term horizon view below, NOT by the overall verdict.
...
export function computeVerdict(inputs = {}) {
  ...
  // Validity floor aligned with the ledger's own logging rule (< 4 scorable
  // categories is never logged): a verdict banded from 2–3 categories reads
  // as a real call while resting on a fraction of the formula — keyless
  // installs were showing HIGHER scores than keyed ones for the same stock
  // because the missing categories (valuation, momentum, analysts) are the
  // score-lowering ones for large caps. Bands and weights are unchanged; no
  // logged v2 call had fewer than 4 categories, so nothing graded shifts.
  if (availableCount < 4) {
    return {
