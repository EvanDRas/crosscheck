...
function readAll() {
  try {
    ...
    // A missing file is normal; an UNPARSEABLE one is data loss — park it
    // so the failure is at least visible on disk instead of silent.
    if (err?.code !== "ENOENT") {
...
export function takeSnapshot(payload) {
  ...
  return {
    ...
    // The share price at snapshot time — the root cause behind most score
    // drift, and the only channel through which news ever reaches the score.
    price: typeof payload.quote?.price === "number" && Number.isFinite(payload.quote.price) ? payload.quote.price : null,
    ...
    drivers: (() => {
      ...
      // The scorer excludes negative P/E, PEG, and D/E as meaningless; the
      // narrator must too, or a loss-to-profit flip reads as the opposite
      // of what happened ("price grew faster than earnings" on -12× → 25×).
      const posOnly = (v) => (v !== null && v > 0 ? v : null);
      const nonNeg = (v) => (v !== null && v >= 0 ? v : null);
      return {
        pe: posOnly(pick("pe")), ps: pick("ps"), peg: posOnly(pick("peg")),
        netMargin: pick("netMargin"), roe: pick("roe"),
        revenueGrowth: pick("revenueGrowth"), epsGrowth: pick("epsGrowth"),
        currentRatio: pick("currentRatio"), debtEquity: nonNeg(pick("debtEquity")),
...
// Which real-world event each driver answers to. Price-channel drivers move
// when the SHARE PRICE moves; fundamentals drivers move when a NEW FILING
// lands; the analyst driver is its own cause.
const PRICE_CHANNEL = new Set(["pe", "ps", "peg", "pricePosition"]);
const FILING_CHANNEL = new Set(["netMargin", "roe", "revenueGrowth", "epsGrowth", "currentRatio", "debtEquity"]);

function explainCategoryMove(label, prevDrivers, currDrivers, catDelta, cause = {}) {
  ...
  for (const [key, fmt, story] of stories) {
    ...
    // Old snapshots may still carry negative ratios — refuse to narrate them.
    if ((key === "pe" || key === "peg") && (a <= 0 || b <= 0)) continue;
    if (key === "debtEquity" && (a < 0 || b < 0)) continue;
    ...
    if (!best || rel > best.rel) best = { rel, key, text: `${story(b > a)} (${fmt(a)} \u2192 ${fmt(b)})` };
  }
  if (!best) return "";
  // Chain to the root cause when we can see it.
  let root = "";
  if (PRICE_CHANNEL.has(best.key) && typeof cause.priceChgPct === "number" && Math.abs(cause.priceChgPct) >= 1.5) {
    root = `, with the share price ${cause.priceChgPct > 0 ? "up" : "down"} ${Math.abs(cause.priceChgPct).toFixed(1)}% since your last look`;
  } else if (FILING_CHANNEL.has(best.key) && cause.newFiling) {
    root = ` \u2014 fresh numbers from the ${cause.newFiling.form} filed ${cause.newFiling.filed}`;
  }
  return ` \u2014 ${best.text}${root}`;
}

// A filing is news when the date advanced OR a different form landed on the
// same date (an 8-K often files alongside the 10-Q it accompanies).
const filingIsNew = (p, c) => Boolean(c && (!p || c.filed > p.filed || (c.filed === p.filed && c.form !== p.form)));

// Scores are stored at one decimal; display them the same way so "41.5"
// never shows as "42" beside a band table that read the 41.5.
const fmtScore = (v) => (v % 1 === 0 ? String(v) : v.toFixed(1));
...
export function diffSnapshots(prev, curr) {
  ...
  if (num(prev.score) && num(curr.score) && Math.abs(curr.score - prev.score) >= 1) {
    ...
    // The real-world causes visible in the window: how far the share price
    // moved, and whether a new filing landed.
    const cause = {
      priceChgPct: num(prev.price) && num(curr.price) && prev.price > 0 ? ((curr.price - prev.price) / prev.price) * 100 : null,
      newFiling: filingIsNew(prev.latestFiling, curr.latestFiling) ? curr.latestFiling : null,
    };
    let why = "";
    if (prev.cats && curr.cats) {
      const movers = Object.keys(curr.cats)
        ...
        .slice(0, 2)
        .map((k) => `${k} ${prev.cats[k]} → ${curr.cats[k]}${explainCategoryMove(k, prev.drivers, curr.drivers, curr.cats[k] - prev.cats[k], cause)}`);
      if (movers.length) why = `: ${movers.join("; ")}`;
      else {
        // A category gaining or losing data shifts every weight — that IS
        // the story, and "sub-point drifts" would be a lie about it.
        const gone = Object.keys(prev.cats).filter((k) => !(k in curr.cats));
        const came = Object.keys(curr.cats).filter((k) => !(k in prev.cats));
        if (gone.length || came.length) {
          const bits = [...gone.map((k) => `${k} had no data this time`), ...came.map((k) => `${k} has data again`)];
          why = ` — ${bits.join("; ")}, so the category weights renormalized`;
        } else why = " — a sum of sub-point drifts across categories, nothing decisive";
      }
    }
    if (!why) {
      ...
      for (const [key, a, b, label] of legacy) {
        ...
        if (key === "pe" && (a <= 0 || b <= 0)) continue; // negative P/E is not a ratio worth narrating
        const implied = BAD_WHEN_UP.has(key) ? b < a : b > a;
    ...
    out.push(`Score ${fmtScore(prev.score)} → ${fmtScore(curr.score)}${verdictBit}${why}`);
  ...
  if (filingIsNew(prev.latestFiling, curr.latestFiling)) {
...
export function updateSnapshot(ticker, payload) {
  ...
  if (!prev || prevDay !== currDay) {
    ...
    try {
      ...
      // tmp+rename: a crash mid-write must not silently wipe every ticker's
      // "since you last looked" baseline.
