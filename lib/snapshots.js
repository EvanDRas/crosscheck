  ...
  return {
    ...
    drivers: (() => {
      ...
      return {
        ...
        pricePosition: pick("pricePosition"), analystTilt: pick("analystTilt"),
      };
    })(),
    pe: m.pe ?? null,
...
function explainCategoryMove(label, prevDrivers, currDrivers) {
  ...
  if (num(prev.score) && num(curr.score) && Math.abs(curr.score - prev.score) >= 1) {
    ...
    if (prev.cats && curr.cats) {
      const movers = Object.keys(curr.cats)
        ...
        .slice(0, 2)
        .map((k) => `${k} ${prev.cats[k]} → ${curr.cats[k]}${explainCategoryMove(k, prev.drivers, curr.drivers)}`);
