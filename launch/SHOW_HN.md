...
- You can log your OWN calls ("I'd buy this here") and it grades you the
  ...
  the whole point.
- The demo to try: the "Time machine" chip on the landing page (needs the
  free Tiingo key) reruns NVDA on 2023-01-03 using only what was knowable
  that day — the formula said SELL; the stock then did over +1,400%. You can
  run that on any ticker and any date back to 2010. The formula can't hide
  from its own history, and that's the feature.
- There's also a Daily 5: five mystery stocks a day, tickers hidden, same
  five for everyone — you call beat-or-trail vs the S&P, then see what the
  formula said with the same numbers. It loses regularly. That's the thesis
  as a game.
